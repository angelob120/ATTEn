        ...
        VStack {
            ...
            VStack {
                ...
                GeometryReader { geometry in
                    HStack(spacing: 0) {
                        MentorWeekView(dates: weekDays(weekOffset: weekOffset),
                                       selectedDate: $selectedDate,
                                       dateFormatter: dateFormatter,
                                       dayOfWeekFormatter: dayOfWeekFormatter)
                            .frame(width: geometry.size.width)
                        
                        MentorWeekView(dates: weekDays(weekOffset: weekOffset + (dragOffset < 0 ? 1 : -1)),
                                       selectedDate: $selectedDate,
                                       dateFormatter: dateFormatter,
                                       dayOfWeekFormatter: dayOfWeekFormatter)
                            .frame(width: geometry.size.width)
            ...
            VStack {
                ZStack {
                    Circle()
                        ...
                        .stroke(isPunchedIn ? Color.blue : Color.gray, lineWidth: 12)
                ...
                Button(action: togglePunch) {
                    Text(isPunchedIn ? "Punch Out" : "Punch In")
                        ...
                        .padding()
                        .frame(width: 200)
                        .background(isPunchedIn ? Color.red : Color.blue)
                        .cornerRadius(8)
                }
            }
            .padding(.top, 24)
            ...
            ForEach(dates, id: \.self) { date in
                VStack {
                    ...
                    Text(dateFormatter.string(from: date))
                        .font(.headline)
                        .padding(10)
                        .background(isSameDay(date1: selectedDate, date2: date) ? Color.blue : Color.clear)
                        .foregroundColor(isSameDay(date1: selectedDate, date2: date) ? .white : (isSameDay(date1: Date(), date2: date) ? Color.blue : .primary))
                        .clipShape(Circle())
                }
                .onTapGesture {
                    selectedDate = date
...
struct MentorHomeView_Previews: PreviewProvider {
